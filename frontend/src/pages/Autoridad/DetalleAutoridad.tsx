...
export default function DetalleAutoridad() {
  ...
  // Handler para derivar solo a Dirgegen
  const handleDerivacion = async (observacion: string, _tipoDerivacion?: TipoDerivacionVRA) => {
    if (!denuncia) return;
    try {
      setProcessing(true);
      const idDenuncia = denuncia.ID_Denuncia;

      // Solo se permite derivar a Dirgegen
      const nuevoTipoId = 303; // Tipo para derivación a Dirgegen
      const mensajeExito = 'Denuncia derivada exitosamente a Dirgegen.';
