...
export default function DetalleDirgegen() {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        'Denunciante participantes_caso:',
        (data as any)?.denunciante?.participantes_caso
      );
  ...
  // Solicitudes
  const solicitudesDeMedida: SolicitudMedida[] =
    denuncia.solicitudes_medidas || denuncia.SolicitudesMedidas || [];
    ...
    <section className="mx-auto max-w-6xl pb-12 px-4 py-8 space-y-6">
      ...
      <div className="space-y-4">
        {solicitudesDeMedida.length > 0 && (
          <div
            className={`rounded-lg border shadow-sm overflow-hidden ${solicitudPendiente ? 'border-red-200 bg-red-50' : 'border-blue-200 bg-white'}`}
          >
            <div className="px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2
                    className={`text-sm font-bold uppercase tracking-wider ${solicitudPendiente ? 'text-red-800' : 'text-blue-800'}`}
                  >
                    {solicitudPendiente ? 'Atención: Medida Pendiente' : 'Historial de Medidas'}
                  </h2>
                  {solicitudPendiente && (
                    <p className="text-xs text-red-600 mt-1">Se requiere revisar la solicitud.</p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="rounded-lg border border-green-200 bg-green-50 p-4 flex items-center gap-3 shadow-sm">
            ...
            <div>
              ...
              <p className="text-xs text-green-700">
                El caso cuenta con documentación técnica finalizada.
              </p>
            </div>
          </div>
        )}
          ...
          {esRangoFechas ? (
            <p className="text-sm text-gray-600 mt-1">
              ...
              <span className="font-medium">
                Del {formatDate(fechaHechos)} al {formatDate(fechaFinCaso)}
              </span>
            </p>
          ) : (
            <p className="text-sm text-gray-600 mt-1">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          {tieneInforme ? (
            <button
              ...
              className="px-5 py-2.5 bg-white border border-green-500 text-green-700 rounded-lg text-sm font-bold hover:bg-green-50 shadow-sm transition-all"
            >
              📄 Ver Informe
            </button>
          ) : (
