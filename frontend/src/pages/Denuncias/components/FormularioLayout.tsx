        ...
        <section className="mx-auto max-w-4xl py-6">
            ...
            <div className="mt-6 flex flex-col gap-3 border-t border-gray-100 pt-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-col gap-2 md:flex-row">
                    <button type="button" onClick={onBack} className="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">
                        {step === 1 ? 'Volver a selección' : 'Volver'}
                    </button>

                    {step < steps.length && (
                        <button type="button" disabled={!puedeAvanzar} onClick={onNext} className="rounded-md bg-ubb-blue px-4 py-2 text-sm font-semibold text-white hover:opacity-95 disabled:opacity-50 disabled:cursor-not-allowed">
                            Siguiente
                        </button>
                    )}

                    {step === steps.length && (
                        <button type="button" onClick={onSend} disabled={enviando} className="rounded-md bg-ubb-blue px-4 py-2 text-sm font-semibold text-white hover:opacity-95 disabled:opacity-50">
