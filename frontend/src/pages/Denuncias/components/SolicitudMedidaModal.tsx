...
export default function SolicitudMedidaModal({
  idDenuncia,
  isOpen,
  onClose,
  onSuccess,
}: SolicitudMedidaModalProps) {
      ...
      <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div className="relative rounded-lg bg-white p-6 shadow-xl transition-all sm:my-8 w-full max-w-md">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Esta solicitud será derivada a la unidad competente (
            {idDenuncia < 200 ? 'Dirgegen' : 'VRA/Fiscalía'}) para su evaluación urgente.
          </p>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700">
                Tipo de Medida Requerida *
              </label>
              <select
                value={tipoMedida}
