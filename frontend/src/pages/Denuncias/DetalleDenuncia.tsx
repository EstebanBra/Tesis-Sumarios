...
export default function DetalleDenuncia() {
  ...
  const carreraCargoDenunciante =
    getProp(datosDenuncianteObj, 'carreraCargo', 'Carrera_Cargo') ||
    getProp(datosDenuncianteObj, 'Carrera_Cargo', 'carreraCargo');
  ...
  const rutVictima = esVictima ? rutDenunciante : victimaExterna?.persona?.Rut || null;

  const correoVictima = esVictima ? correoDenunciante : victimaExterna?.persona?.Correo || null;

  const telefonoVictima = esVictima
    ? telefonoDenunciante
    : victimaExterna?.persona?.Telefono || null;

  const sexoVictima = esVictima ? sexoDenunciante : victimaExterna?.persona?.sexo || null;

  const generoVictima = esVictima ? generoDenunciante : victimaExterna?.persona?.genero || null;
  ...
  // Archivos/Evidencias
  const archivosRaw =
    denuncia?.archivos_denuncia ||
    denuncia?.denunciante?.participantes_caso?.flatMap(
      (pc: any) => pc.hitos?.flatMap((hito: any) => hito.archivos || []) || []
    ) ||
    [] ||
    denuncia?.Evidencias ||
    denuncia?.evidencias ||
  ...
  const pasosProceso = [
    {
      ...
      nombre: 'Recepción',
      plazo: 'Día 0',
      desc: 'Ingreso inmediato',
      claves: ['pendiente', 'recibida', 'ingresada'],
    },
    {
      id: 2,
      nombre: 'Admisibilidad',
      plazo: '3-5 días hábiles',
      desc: 'Análisis de forma',
      claves: ['revisión', 'admisibilidad', 'análisis'],
    },
    {
      id: 3,
      nombre: 'Investigación',
      plazo: 'Hasta 20 días hábiles',
      desc: 'Indagatoria y Fiscal',
      claves: ['investigación', 'fiscal', 'sumario'],
    },
    {
      id: 4,
      nombre: 'Resolución',
      plazo: '5-10 días hábiles',
      desc: 'Sanción o Cierre',
      claves: ['resuelta', 'cerrada', 'sanción', 'finalizada'],
    },
  ];
  ...
  const diasTranscurridos = fechaIngreso
    ? Math.floor((new Date().getTime() - new Date(fechaIngreso).getTime()) / (1000 * 60 * 60 * 24))
    : 0;
    ...
    <section className="mx-auto max-w-6xl pb-12 px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200 pb-4 pt-2">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">
            Caso #{denuncia.ID_Denuncia || denuncia.id}
          </h1>
          <p className="text-sm text-gray-500 mt-1">Fecha de Ingreso: {formatDate(fechaIngreso)}</p>
          {esRangoFechas ? (
            <p className="text-sm text-gray-600 mt-1">
              Fecha de los hechos:{' '}
              <span className="font-medium">
                Del {formatDate(fechaHechos)} al {formatDate(fechaFinRaw)}
              </span>
            </p>
          ) : (
            <p className="text-sm text-gray-600 mt-1">
      ...
      {/* <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-200">
        <div className="flex justify-between items-center mb-8 border-b border-gray-100 pb-4">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              Tiempo transcurrido:{' '}
              <span className="font-semibold text-indigo-600">{diasTranscurridos} días</span>
            </p>
          </div>
          <span className="text-xs font-semibold text-gray-400 bg-gray-50 px-2 py-1 rounded border border-gray-200">
            Reglamento DUE 4560 / 5415
          </span>
        </div>
            ...
            {pasosProceso.map((paso, idx) => {
              ...
              return (
                <div key={paso.id} className="flex flex-col items-center w-32 text-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm border-2 bg-white transition-all
                  ...
                  <div className="mt-3">
                    <span
                      className={`text-sm font-bold block ${isActive ? 'text-gray-900' : 'text-gray-400'}`}
                    >
                      {paso.nombre}
                    </span>
        ...
        <div className="mt-6 bg-blue-50 p-3 rounded text-xs text-blue-800 flex gap-2">
          <span>ℹ️</span>{' '}
          <p>
            Plazos en días hábiles administrativos. La etapa de investigación puede prorrogarse
            según complejidad.
          </p>
        </div>
      </div> */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            if (esCampoClinico) {
                ...
                if (partes.length > 0) {
                  ...
                  if (!comunaEstablecimiento && partes.length > 2) {
                    ...
                    for (let i = 1; i < partes.length - 1; i++) {
                      const parte = partes[i];
                      if (
                        parte.toLowerCase() !== regionEstablecimiento?.toLowerCase() &&
                  ...
                  <div className="space-y-3">
                    ...
                    {(regionEstablecimiento || comunaEstablecimiento) && (
                      <div>
                        ...
                        <p className="text-sm font-medium text-gray-900 mt-1">
                          {[comunaEstablecimiento, regionEstablecimiento]
                            .filter(Boolean)
                            .join(', ')}
                        </p>
                      </div>
              ...
              if (!sedeNombre && !lugarNombre && !detalleHecho && ubicacion) {
                ...
                if (partes.length > 0) {
                  ...
                  if (sedeNombre) {
                    const regionMatch = sedeNombre.match(
                      /Región\s+(?:de\s+)?(?:del\s+)?([^-()]+)|(?:Región\s+)?([IVX]+)\s+Región/i
                    );
              ...
              if (!regionNombre && ubicacion) {
                const regionMatch = ubicacion.match(
                  /([IVX]+\s*Región\s*(?:de\s+)?(?:del\s+)?[^-()]+)/i
                );
                if (regionMatch) {
                  ...
                  const posiblesRegiones = [
                    {
                      pattern: /(VIII|8)\s*Región\s*(?:de\s+)?(?:del\s+)?(Bío-Bío|Biobío|Bio-Bio)/i,
                      nombre: 'VIII Región del Bío-Bío',
                    },
                    {
                      pattern: /(XVI|16)\s*Región\s*(?:de\s+)?(Ñuble|Nuble)/i,
                      nombre: 'XVI Región de Ñuble',
                    },
                    {
                      pattern:
                        /(XIII|13)\s*Región\s*(?:de\s+)?(?:del\s+)?(Metropolitana|Metropolitana de Santiago)/i,
                      nombre: 'Región Metropolitana',
                    },
                    {
                      pattern: /(V|5)\s*Región\s*(?:de\s+)?(Valparaíso|Valparaiso)/i,
                          ...
                          <div className="w-full">
                            ...
                            {(inv.persona?.sexo || inv.persona?.genero) && (
                              <div className="mt-2 bg-blue-50 border border-blue-200 rounded p-2 text-xs">
                                {inv.persona?.sexo && (
                                  <p>
                                    <span className="font-semibold">Sexo:</span> {inv.persona.sexo}
                                  </p>
                                )}
                                {inv.persona?.genero && (
                                  <p>
                                    <span className="font-semibold">Género:</span>{' '}
                                    {inv.persona.genero === 'NoLoSe'
                                      ? 'No lo sé'
                                      : inv.persona.genero === 'NoBinario'
                                        ? 'No Binario'
                                        : inv.persona.genero}
                                  </p>
                                )}
                            ...
                            {inv.Descripcion || inv.descripcion ? (
                              ...
                              <p className="text-sm text-gray-600 mt-1">
                                <span className="font-semibold">Vinculación:</span>{' '}
                                {inv.Vinculacion || inv.vinculacion || 'No especificada'}
                              </p>
                            )}
                          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
                ...
                <>
                  <div>
                    ...
                    <p className="font-medium text-gray-900">
                      {nombreCompletoDenunciante || 'No especificado'}
                    </p>
                    {rutDenunciante && (
                  ...
                  <div className="pt-2 border-t border-gray-100">
                    ...
                    <p className="font-mono text-sm font-medium text-gray-900">
                      {rutDenunciante || 'No especificado'}
                    </p>
                  </div>
                  <div className="pt-2 border-t border-gray-100">
                    <label className="text-xs text-gray-400 uppercase font-bold block mb-1">
                    ...
                    {telefonoDenunciante && <p className="text-gray-700">{telefonoDenunciante}</p>}
                    ...
                    <div className="pt-2 border-t border-gray-100">
                      ...
                      {(regionDenunciante || comunaDenunciante) && (
                        <p className="text-gray-700 text-xs">
                          ...
                          {comunaDenunciante && (
                            <>
                              , <strong>Comuna:</strong> {comunaDenunciante}
                            </>
                          )}
                        </p>
                      )}
                      {direccionDenunciante && (
                        <p className="text-gray-700 text-xs">
                          <strong>Domicilio:</strong> {direccionDenunciante}
                        </p>
                      )}
                    </div>
                  )}
          ...
          {/* CARD: VÍCTIMA */}
          <div
            className={`rounded-xl shadow-sm border overflow-hidden ${victimaMenor ? 'border-red-300 bg-red-50' : 'border-gray-200 bg-white'}`}
          >
            <div
              className={`px-5 py-3 border-b flex justify-between items-center ${victimaMenor ? 'bg-red-100 border-red-200' : 'bg-gray-50 border-gray-200'}`}
            >
              <h3
                className={`font-bold text-sm ${victimaMenor ? 'text-red-800' : 'text-gray-800'}`}
              >
                  ...
                  {!rutVictima && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mt-3">
                      <p className="text-xs text-yellow-800">
                        ⚠️ <strong>Nota:</strong> La víctima aún no ha sido completamente
